import { createMemoryHistory, createRouter, type MatchResult, type RouteConfig } from '@diso.io/groutcho';
...
const routes: Record<string, RouteConfig> = {
  ...
  Show: { pattern: '/show/:title', page: (m: MatchResult) => html`<div id="show">${m.params.title}</div>` }
};
...
describe('groutcho-lit elements', () => {
  ...
  it('navigates when a link is clicked', async () => {
    const store = createRouter({ routes, history: createMemoryHistory('/show/start') });
    const outlet = mount<GroutchoOutlet>('groutcho-outlet');
    outlet.store = store;
