import { createMemoryHistory, type RouteConfig, type RedirectTest } from '@diso.io/groutcho';
import { render, screen, fireEvent, cleanup } from '@testing-library/react';
import { afterEach, describe, expect, it } from 'vitest';

import { RouterProvider, Link, useMatch } from '../src/index';
...
describe('groutcho-react', () => {
  it('renders the current route via useMatch', () => {
    render(
      <RouterProvider routes={routes} redirects={redirects} history={createMemoryHistory('/show/hi')}>
        <CurrentRoute />
      </RouterProvider>
    );
  ...
  it('supports the render-prop (RouterContainer back-compat) shape', () => {
    render(
      <RouterProvider routes={routes} redirects={redirects} history={createMemoryHistory('/')}>
  ...
  it('follows redirects from the store on initial render', () => {
    render(
      <RouterProvider routes={routes} redirects={redirects} history={createMemoryHistory('/dashboard')}>
        <CurrentRoute />
      </RouterProvider>
    );
