...
import { Router, Route, type RouteConfig, type RedirectTest } from '../src/index';
  ...
  describe('Route', () => {
    describe('constructor', () => {
      it('should require all route params', () => {
        expect(() => new Route({ name: 'wow', pattern: '/' } as unknown as RouteConfig)).toThrow();
      });
    ...
    describe('.is', () => {
      it('should test match', () => {
        const route = new Route({ name: 'Derp', pattern: '/derp/:derp', page: Page });
  ...
  describe('Router', () => {
    ...
    describe('.getRouteByName', () => {
      ...
      it('should throw on missing route', () => {
        expect(() => router.getRouteByName('MissingAndPresumedScared')).toThrow();
      });
    });
    ...
    describe('.match', () => {
      ...
      it('should handle missing param', () => {
        const match = router.match({ route: { name: 'OneParam', params: { barf: 'barf' } } });
        expect(match.redirect).toBeTruthy();
        expect(match.url).toBe('/404');
      });
      ...
      it('should handle one param route', () => {
        const match = router.match({ route: { name: 'OneParam', params: { title: 'barf' } } });
        expect(match.route).toBeTruthy();
        expect(match.redirect).toBeFalsy();
        expect(match.route!.name).toBe('OneParam');
      ...
      it('should handle extra params by adding them to the query', () => {
        const match = router.match({
          route: { name: 'TwoParam', params: { foo: 'd', barf: 'b', donk: 'ed', honk: 'y' } }
        });
        expect(match.route).toBeTruthy();
        expect(match.redirect).toBeFalsy();
      ...
      it('should throw on endless redirect loop', () => {
        expect(() => router.match('/endless')).toThrow(/Number of redirects exceeded/);
      });

      it('should handle bad redirect', () => {
        expect(() => router.match('/badredirect')).toThrow(/No match for redirect result/);
      });
