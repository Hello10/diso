...
const URLPatternImpl: typeof URLPatternPolyfill =
  (globalThis as { URLPattern?: typeof URLPatternPolyfill }).URLPattern ?? URLPatternPolyfill;
...
export function compilePath(pattern: string, params: Params): CompiledPath {
  ...
  let path = pattern.replace(SLASH_TOKEN, (_all, name: string, modifier: string) => {
  ...
  path = path.replace(TOKEN, (_all, name: string, modifier?: string) => {
    ...
    if (isEmpty(value)) {
      ...
      throw new Error(`Missing required param "${name}" for pattern "${pattern}"`);
    }
    return encodeValue(value, repeat);
  });
