...
export class Router {
  ...
  getRoute(query: Record<string, unknown>): Route | undefined {
    return this.routes.find((route) =>
      Object.entries(query).every(([key, value]) => (route as Record<string, unknown>)[key] === value)
    );
  }
  ...
  #checkRedirects(args: CheckRedirectsArgs): MatchResult | false {
    ...
    let { previous = null, current = null, num_redirects = 0, history = [] } = args;
    ...
    // `current` may be `false` (no match); `false` has no `.redirect`, so guard.
    ...
    if (next) {
      ...
      if (!current) {
        const error = new Error('No match for redirect result') as Error & { redirect?: unknown };
        error.redirect = normalizedNext;
        throw error;
      }
      history.push(current);
      num_redirects++;
      return this.#checkRedirects({ original, previous, current, num_redirects, history, extra });
    }
