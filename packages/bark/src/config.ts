...
export const pretty: Formatter = (record) => {
  ...
  parts.push({ ...record.fields, name: record.name, time: record.time, ...(record.traceId ? { traceId: record.traceId } : {}) });
  return parts;
};
...
function resolveLevel(): string | string[] {
  ...
  const ls = (globalThis as { localStorage?: Record<string, string> }).localStorage;
  if (ls?.LOGGER) {
    return ls.LOGGER;
  }
  return '*';
...
function resolve(): Resolved {
  ...
  const { format = 'pretty', sink = consoleSink, timing = true, timingAllowOrigin, time } = explicit;
  const formatter = typeof format === 'function' ? format : format === 'json' ? json : pretty;
  resolved = {
