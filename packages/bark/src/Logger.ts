...
import { type Fields, LEVELS, type Level } from "./record";
import { serializeError, serializeFields, serializers } from "./serialize";
...
export class Bark {
	...
	branch(name: string, fields: Fields = {}): Bark {
		...
		return new Bark(joined, this.#context, {
			...this.#fields,
			...serializeFields(fields),
		});
	}
	...
	finish(fields: Fields = {}): Response | undefined {
		...
		let response =
			fields.response instanceof Response ? fields.response : undefined;
	...
	#emit(level: Level, message?: string | Error, fields?: Fields): void {
		...
		if (isError) {
			merged.error = (serializers.get("error") ?? serializeError)(message);
