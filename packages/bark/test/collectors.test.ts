...
import Bark, { resetCollectors, serverTimingHeader, type BarkRecord, type Span } from '../src/index';
...
describe('browser collectors (Bark.start in a document)', () => {
  ...
  it('captures global errors as error records on the root scope', () => {
    ...
    expect((record.fields.error as { fingerprint: string }).fingerprint).toBeTruthy();
  });
...
describe('round-trip correlation (server spans -> Server-Timing -> client)', () => {
  ...
  function parseServerTiming(header: string): Array<{ name: string; dur: number; desc?: string }> {
    return header.split(',').map((part) => {
      const segments = part.trim().split(';');
      const out: { name: string; dur: number; desc?: string } = { name: segments[0]!, dur: 0 };
