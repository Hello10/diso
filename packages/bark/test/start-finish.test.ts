...
import Bark, { type BarkRecord, parseTraceparent } from "../src/index";
...
describe("Bark.start / finish (server)", () => {
	...
	it("span end is idempotent", () => {
		...
		span.end();
		span.end();
		expect(records.filter((record) => record.kind === "timing")).toHaveLength(
			1,
		);
	});
	...
	it("works Workers-shaped: explicit config, json lines, no env reliance", () => {
		...
		const bark = Bark.start(
			{ request: new Request("https://x.dev/api") },
			"worker",
		);
		bark.set({ userId: "u42" });
		bark.info("handled");
